...
public interface ReleaseRepository extends JpaRepository<ClientReqForm, Long> {
    ...
    @Query("SELECT COALESCE(MAX(CAST(SUBSTRING(r.ldControlNumber, LENGTH(CONCAT(:year, '/', LPAD(:month, 2, '0'), '/')) + 1, 4) AS INTEGER)), 0) " +
           "FROM ClientReqForm r " +
           "WHERE FUNCTION('YEAR', r.submitDate) = :year " +
           "AND FUNCTION('MONTH', r.submitDate) = :month")
